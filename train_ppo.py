#!/usr/bin/env python3
"""
PPO Training Script for Dynamic Parking Pricing

Compares PPO vs A2C performance
Expected: PPO should achieve 10-14% improvement
"""
...
from role_2.ppo_from_scratch import PPOAgent, PPOConfig

print("[INFO] Training PPO agent for parking pricing")
print("=" * 80)

# Setup
results_dir = Path("./training_results_ppo")
...
config = PPOConfig(
    ...
    hidden_dim=256,
    num_hidden_layers=2,
    policy_lr=1e-3,       # INCREASED from 3e-4
    value_lr=5e-3,        # INCREASED from 1e-3
    gamma=0.99,
    gae_lambda=0.98,      # INCREASED from 0.95 (better returns)
    clip_ratio=0.3,       # INCREASED from 0.2 (allow larger updates)
    entropy_coef=0.02,    # INCREASED from 0.01 (more exploration)
    value_loss_coef=1.0,  # INCREASED from 0.5 (balance objectives)
    max_grad_norm=1.0,    # INCREASED from 0.5
    l2_reg=0.0,           # REMOVED (interferes with continuous)
    batch_size=64,
    num_epochs=1,         # REDUCED from 4 (no overfitting)
    device='cpu'
)
...
print("      - Tuned for continuous control")
...
for episode in range(1, 501):
    ...
    # Train after episode (PPO update with multiple epochs)
    if len(states) > 0:
...
# Save agent
agent.save(str(results_dir / "ppo_agent.pkl"))
print(f"Agent saved to: {results_dir / 'ppo_agent.pkl'}")
